...
export function formatPropertyValue(value: any, propKey?: string): string {
  ...
  if (typeof value === "string") {
    // In Studio there is no /view/<ark> route, so render ark identifiers as plain text.
...
export function getDisplayableProperties(
  entityData: RawGraphEntity | Record<string, never> | undefined
): Record<string, any> {
  const excludeKeys = [
    "@id",
    "@type",
...
export class GraphBuilder {
  ...
  buildInitialGraph(depth = 2): GraphElements {
