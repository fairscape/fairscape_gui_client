...
const ViewerWrapper = styled.div`
  ...
  color: #1a1a1a; /* dark text on the light viewer surface (nodes, legend) */
...
const GraphRenderer: React.FC<GraphRendererProps> = ({
  ...
  const styledEdges = edges.map((edge) => {
    ...
    const typeClass = `edge-${String(edge.label ?? "").replace(/\s+/g, "-")}`;
...
interface EvidenceGraphViewerProps {
  ...
  /** Optional: jump to an entity (used to cross-link into the Explore tab). */
