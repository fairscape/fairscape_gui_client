...
import { SAFE_TOOLS, gradingWritePath, initialPrompt, nextId } from './shared';
...
/**
 * Drives one run of /fairscape-rocrate-wizard via the Claude Agent SDK and bridges its
 * I/O to the renderer: streams activity, routes AskUserQuestion to option-card UI, gates
 * tool permissions, and feeds plain-text answers back as new turns.
 */
export class ClaudeEngine implements AgentEngine {
  ...
  private isGradingWrite(toolName: string, input: unknown): string | null {
    ...
    return gradingWritePath(this.config.dir, fp);
  }
  ...
  /**
   * Interrupt the running turn and inject `text` as the next user turn. Any pending
   * permission/question prompts are settled (denied) first so their promises don't hang;
   * the session itself stays alive, so the wizard picks the new instruction up with full
   * context and resumes the flow.
   */
...
class TurnQueue {
  ...
  iterable(): AsyncIterable<SDKUserMessage> {
    const next = (): Promise<IteratorResult<SDKUserMessage>> => {
      ...
      if (this.done) return Promise.resolve({ value: undefined as unknown as SDKUserMessage, done: true });
